...
if (!session_check()) {
    ...
    $assigned_vars["be_events"] = $db->getAll("SELECT c.*,r.name as route, r.exam, r.bdate as date, r.height, cm.fan,cl.city,cl.name as club, cl.avatar, cl.id as club_id, cm.fan_riders, r.type
													...
													ORDER BY bdate ASC", $user_id, $user_id, date('Y-m-d H:i:s'));
    ...
    $my_fans = $db->getAll("SELECT COUNT(cm.id) as count,cm.cid FROM `users` as u
                            ...
                            WHERE u.id = ?i AND c.bdate > ?s GROUP BY cm.cid", $user_id, date('Y-m-d'));
    $my_fans_ar = array();
    ...
    $assigned_vars["another_user"] = template_get_user_info($user_id); //many info
    $assigned_vars["page_title"] = "Мероприятия ".$assigned_vars["another_user"]['fio']." > Одноконники";
    template_render($assigned_vars, "user-events.tpl");
}
